...
@app.get("/")
async def read_root():
    return {"message": "Welcome to the German Song Lyrics API"}

@app.post("/api/agent")
async def get_lyrics(request: LyricsRequest) -> Dict[str, Any]:
    logger.info(f"Received request: {request.message_request}")
    try:
        # Check if the lyrics are in German
        language = langdetect.detect(request.message_request)
        if language != 'de':
            logger.error("Only German songs are supported.")
            raise HTTPException(status_code=400, detail="Only German songs are supported.")
