...
async def get_page_content(url: str) -> Dict[str, Optional[str]]:
    ...
    Returns:
        Dict[str, Optional[str]]: Dictionary containing lyrics and metadata
    """
...
def extract_lyrics_from_html(html: str, url: str) -> Dict[str, Optional[str]]:
    ...
    lyrics_patterns = [
        # Class patterns
        {"class_": re.compile(r"lyrics?|original", re.I)},
        {"class_": re.compile(r"song-content|song-text|track-text", re.I)},
        # ID patterns
        {"id": re.compile(r"lyrics?|original", re.I)},
    ]
    ...
    # Log the results
    if lyrics:
        logger.info(f"Found lyrics ({len(lyrics)} chars)")
